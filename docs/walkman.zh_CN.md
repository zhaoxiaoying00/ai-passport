...
14 px 文楷字库覆盖基本平面的中日韩汉字，支持生成式回应。联网回应从第一页开始，每页最多三行，均使用相同颜色与 14 px 字号。新文字到达不会把页面推到末尾。对话模型先生成文字，再由 `qwen-audio-3.0-tts-plus` 使用相同的 `longanlingxin` 音色合成，并返回逐字时间戳。播放采样数到达下一页新增文字的朗读时间后整页切换，并补偿声卡 DMA 队列的 60 毫秒；缺失时间戳时不会按猜测字速翻页。两个 TLS 连接依次开启，语音合成会单独产生服务用量。主动关闭套接字时立即唤醒读取轮询，省去最多一秒的退出等待。播报结束后先释放合成连接及播放缓冲，再提前准备一个对话连接，最多保留二十秒；不会开启麦克风或生成回应。准备失败不影响已完成的回应，下一次明确操作可正常重连。连续下一轮可复用已准备的连接。文字仍随返回立即显示，不通过延迟显示来掩盖网络等待。最近文字上下文仅保留在 2 KiB 有界内存中，重启清空。恢复历史对话时逐条发送并等待服务端确认，全部确认后才开始录音，避免历史消息与麦克风上传争用通信内存。换行时将标点与前面的文字放在一起，避免行首出现标点；开引号与后面的文字保持相连，字库补齐弯引号、破折号和省略号。底部只显示翻页提示和音量，不显示“自动”或页码。过短的末行会与前行均衡换行，每页连续显示三行，不重复上一页的文字；仅最后一页允许不足三行，并在固定字幕区域内垂直居中。播放结束不会强制跳到末页。上下键整页翻阅，播放中手动操作会暂停自动翻页六秒；播放结束后停留在手动选择的页码，直到下一次回应。限定长度内的中英文混合长回复均可完整翻阅。本地夸夸仍使用同步字幕，通用文案不包含个人姓名。合成音频通过 WebSocket 二进制分片流式接收，使用 4 KiB 元数据缓冲和容量固定的 PCM 队列；累计逐字时间戳逐个解析，只保留翻页位置，不在内存中保存整包编码音频。传输消息上限为 128 KiB，压缩后的 JSON 元数据上限为 4 KiB。录音每 40 毫秒合并发送一组 PCM，编码后的追加消息可放入一个 2 KiB TLS 输出记录和 2 KiB WebSocket 缓冲；语音合成仍单独使用 4608 字节 WebSocket 缓冲，降低录音时的内存压力。TCP 待发送缓冲限定为 5760 字节，工作任务先处理少量积压录音包再让出执行时间。录音队列满时提示上传失败，不将其判断为 Wi-Fi 信号问题；WebSocket 收发缓冲在连接前预留，配合独立发送锁和 TCP_NODELAY；PCM 直接编码到上传消息，省去另一份原始录音副本。Wi-Fi 缓冲数量受限，附近网络列表仅在配网时分配内存。
